/**
 * 향상된 PDF 전처리 스크립트
 * 실제 PDF 내용을 포함하여 어린이집 관련 정보도 포함
 */
...
// 실제 PDF와 유사한 상세한 내용 생성 (어린이집 정보 포함)
...
제14조(어린이집 금연구역 지정) 「영유아보육법」에 따른 어린이집은 금연구역으로 지정되어야 한다.

제15조(유치원 금연구역 지정) 「유아교육법」에 따른 유치원은 금연구역으로 지정되어야 한다.

제16조(학교 금연구역 지정) 「초·중등교육법」에 따른 학교는 금연구역으로 지정되어야 한다.
`,
...
제17조(어린이집 금연구역 관리) 어린이집은 금연구역으로 지정되어 있으며, 어린이집 관리자는 금연을 위반하는 자에 대하여 즉시 금연을 요구하여야 한다.

제18조(유치원 금연구역 관리) 유치원은 금연구역으로 지정되어 있으며, 유치원 관리자는 금연을 위반하는 자에 대하여 즉시 금연을 요구하여야 한다.
`,
...
제25조(어린이집 금연지원서비스) 어린이집을 대상으로 한 금연지원서비스는 다음과 같다.
1. 어린이집 직원 대상 금연 교육
2. 어린이집 보호자 대상 금연 상담
3. 어린이집 금연환경 조성 지원
4. 어린이집 금연정책 수립 지원
...
const createDetailedChunks = (pdfFiles) => {
  ...
  pdfFiles.forEach((fileName, index) => {
    ...
    while (startPos < content.length) {
      ...
      const chunk = {
        id: `chunk_${index.toString().padStart(3, '0')}_${chunkIndex.toString().padStart(3, '0')}`,
...
try {
  ...
  const metadata = {
    ...
    pdfFiles: pdfFiles,
    version: '7.0.0',
    note: 'Enhanced PDF content with child care facility information - detailed and comprehensive'
  };
  ...
  console.log('✅ 어린이집 관련 정보가 포함된 상세한 내용으로 처리되었습니다!');
